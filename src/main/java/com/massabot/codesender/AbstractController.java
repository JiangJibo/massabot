...
import com.massabot.codesender.gcode.GcodeCommandCreator;
import com.massabot.codesender.gcode.GcodeUtils;
import com.massabot.codesender.i18n.Localization;
import com.massabot.codesender.listeners.ControllerListener;
import com.massabot.codesender.listeners.ControllerListener.MessageType;
import com.massabot.codesender.listeners.ControllerStatus;
import com.massabot.codesender.listeners.SerialCommunicatorListener;
...
public abstract class AbstractController implements SerialCommunicatorListener, IController {
	...
	/**
	 * Called prior to streaming commands, separate in case you need to be more restrictive about
	 * streaming a file vs. sending a command. throws an exception if not ready.
	 */
	...
	/**
	 * Disable alarm mode and put device into idle state, throw an exception if not supported.
	 */
	@Override
	...
	/**
	 * Request parser state, either print it here or expect it in the response handler. Throw an
	...
	public final AbstractCommunicator comm;
	...
	private ArrayList<GcodeCommand> queuedCommands; // The list of specially queued commands to be
													// sent.
	...
	protected ControlState currentState = ControlState.COMM_DISCONNECTED;
	...
	public Boolean openCommPort(String port, int portRate) throws Exception {
		...
		this.currentState = ControlState.COMM_IDLE;
	...
	/**
	 * Send duration can be one of 3 things: 1. the current running time of a send. 2. the entire
	 * duration of the most recent send. 3. 0 if there has never been a send.
	 */
	@Override
	...
	private enum RowStat {
		TOTAL_ROWS, ROWS_SENT, ROWS_REMAINING
	}
	...
	/**
	 * Creates a gcode command and queues it for send immediately. Note: this is the only place
	 * where a string is sent to the comm.
	 */
	@Override
	...
	/**
	 * This is the only place where commands with an expected 'ok'/'error' response are sent to the
	 * comm - with the exception of command streams.
	 */
	...
	public GcodeCommand createCommand(String gcode) throws Exception {
		...
		/*for (String code : gcode.split(",")) {
			sb.append(this.commandCreator.createCommand(code).getCommandString());
		}*/
		return this.commandCreator.createCommand(gcode);
		// return new GcodeCommand(sb.toString());
	}
	...
	public void beginStreaming() throws Exception {
		...
		if (this.queuedCommands.size() == 0 && this.rawStreamCommands == null && this.streamCommands == null) {
	...
	protected void fileStreamComplete(String filename, boolean success) {
		...
		String duration = com.massabot.codesender.Utils.formattedMillis(this.getSendDuration());
	...
	/**
	 * Notify controller that the next command has completed with response and that the stream is
	 * complete once the last command has finished.
	 */
