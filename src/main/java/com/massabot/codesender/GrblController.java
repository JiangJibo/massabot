...
import static com.massabot.codesender.model.UGSEvent.ControlState.COMM_IDLE;
...
import com.massabot.codesender.listeners.GrblSettingsListener;
import com.massabot.codesender.model.Overrides;
import com.massabot.codesender.model.Position;
...
import com.massabot.codesender.types.GrblFeedbackMessage;
import com.massabot.codesender.types.GrblSettingMessage;
...
public class GrblController extends AbstractController {
	...
	protected void rawResponseHandler(String response) {
		if (GcodeCommand.isOkErrorResponse(response)) {
				...
				if (parts.length == 2) {
					...
					if (StringUtils.isNumeric(code)) {
						...
						if (errorParts != null && errorParts.length >= 3) {
							processed = "error: " + errorParts[1] + ": " + errorParts[2];
						}
					}
				}
			}
			try {
				...
				this.errorMessageForConsole(Localization.getString("controller.error.response") + " <" + processed + ">" + message);
			}
		}
			...
			Logger.getLogger(GrblController.class.getName()).log(Level.CONFIG, "{0} = {1}{2}",
					new Object[] { Localization.getString("controller.log.version"), this.grblVersion, this.grblVersionLetter });
			Logger.getLogger(GrblController.class.getName()).log(Level.CONFIG, "{0} = {1}",
					new Object[] { Localization.getString("controller.log.realtime"), this.capabilities.REAL_TIME });
		}
	...
	protected void cancelSendBeforeEvent() throws Exception {
		...
		if (this.capabilities.JOG_MODE && controllerStatus != null && "jog".equalsIgnoreCase(controllerStatus.getState())) {
				...
				java.awt.EventQueue.invokeLater(new Runnable() {
					...
					public void run() {
						try {
							...
							messageForConsole(Localization.getString("controller.exception.sendingstatus") + ": " + ex.getMessage() + "\n");
							ex.printStackTrace();
						}
					}
				});
	...
	private void sendStateMessageIfChanged(String beforeState, ControlState current) {
		...
		case "idle":
			if (isStreaming()) {
				...
				if (beforeState != null && beforeState.toLowerCase().equals("jog")) {
	...
	private void handleStatusString(final String string) {
		...
		if (isCanceling) {
			if (attemptsRemaining > 0 && lastLocation != null) {
				...
				else if (grblState.equals("Hold") && lastLocation.equals(machineLocation)) {
					try {
						this.issueSoftReset();
					} catch (Exception e) {
		...
		if (machineLocation != null && this.getUnitsCode() != null) {
			Units u = this.getUnitsCode().toUpperCase().equals("G21") ? Units.MM : Units.INCH;
