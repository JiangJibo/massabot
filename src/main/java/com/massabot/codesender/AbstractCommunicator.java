...
import com.massabot.codesender.connection.ConnectionFactory;
...
public abstract class AbstractCommunicator {
	...
	enum SerialCommunicatorEvent {
		COMMAND_SENT, COMMAND_SKIPPED, RAW_RESPONSE, CONSOLE_MESSAGE, VERBOSE_CONSOLE_MESSAGE
	}
	...
	/**
	 * Reset any internal buffers. In case a controller reset was detected call this.
	 */
	...
	private void dispatchListenerEvents(final SerialCommunicatorEvent event, final String string, final GcodeCommand command) {
		if (event == SerialCommunicatorEvent.COMMAND_SENT || event == SerialCommunicatorEvent.COMMAND_SKIPPED) {
			if (command == null)
	...
	private void sendEventToListeners(final SerialCommunicatorEvent event, ArrayList<SerialCommunicatorListener> sclList, String string, GcodeCommand command) {
		switch (event) {
		case COMMAND_SENT:
	...
	/**
	 * If commands complete very fast, like several comments in a row being skipped, then multiple
	 * event handlers could process them out of order. To prevent that from happening we use a
	 * blocking queue to add events in the main thread, and process them in order a single event
	 * thread.
	 */
	...
	private class EventData {
		...
		public EventData(SerialCommunicatorEvent event, ArrayList<SerialCommunicatorListener> sclList, String string, GcodeCommand command) {
