...
public class GcodePreprocessorUtils {
	...
	/**
	 * Searches the command string for an 'f' and replaces the speed value between the 'f' and the
	 * next space with a percentage of that speed. In that way all speed values become a ratio of
	 * the provided speed and don't get overridden with just a fixed speed.
	 */
	...
	/**
	 * Splits a gcode command by each word/argument, doesn't care about spaces. This command is
	 * about the same speed as the string.split(" ") command, but might be a little faster using
	 * precompiled regex.
	 */
	 ...
	 */
	static public List<Point3d> generatePointsAlongArcBDring(final Point3d start, final Point3d end, final Point3d center, boolean clockwise, double R,
			double minArcLength, double arcSegmentLength, PlaneFormatter plane) {
		double radius = R;
	 ...
	 * Generates the points along an arc including the start and end points.
	 */
	static private List<Point3d> generatePointsAlongArcBDring(final Point3d p1, final Point3d p2, final Point3d center, boolean isCw, double radius,
			double startAngle, double sweep, int numPoints, PlaneFormatter plane) {
	 ...
	 */
	static private Point3d convertRToCenter(Point3d start, Point3d end, double radius, boolean absoluteIJK, boolean clockwise, PlaneFormatter plane) {
		double R = radius;
