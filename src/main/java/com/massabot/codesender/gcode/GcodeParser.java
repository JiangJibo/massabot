...
import static com.massabot.codesender.gcode.util.Plane.*;
...
public class GcodeParser implements IGcodeParser {
	 ...
	 *             If the command is too long throw an exception
	 */
	@Override
	...
	/**
	 * Process commend given an initial state. This method will not modify its input parameters.
	 */
	...
	private static PointSegment addArcPointSegment(Point3d nextPoint, boolean clockwise, List<String> args, int line, GcodeState state) {
		...
		Point3d center = GcodePreprocessorUtils.updateCenterWithCommand(args, state.currentPoint, nextPoint, state.inAbsoluteIJKMode, clockwise,
				new PlaneFormatter(state.plane));
	...
	private static GcodeMeta handleGCode(String code, List<String> args, int line, GcodeState state) {
		...
		Point3d nextPoint = GcodePreprocessorUtils.updatePointWithCommand(args, state.currentPoint, state.inAbsoluteMode);
	...
	/**
	 * Applies all command processors to a given command and returns the resulting GCode. Does not
