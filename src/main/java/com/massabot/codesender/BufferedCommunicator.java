...
import static com.massabot.codesender.AbstractCommunicator.SerialCommunicatorEvent.COMMAND_SENT;
import static com.massabot.codesender.AbstractCommunicator.SerialCommunicatorEvent.COMMAND_SKIPPED;
...
public abstract class BufferedCommunicator extends AbstractCommunicator {
	...
	/**
	 * Add command to the command queue outside file mode. This is the only way to send a command to
	 * the comm port without being in file mode. These commands will be sent prior to any queued
	 * stream, they should typically be control commands calculated by the application.
	 */
	@Override
	...
	*/
	...
	public boolean areActiveCommands() {
		return (this.activeCommandList.size() > 0) || (this.commandStream != null && this.commandStream.getNumRowsRemaining() > 0);
	}
	 ...
	 * SYNCHRONIZED.
	 * 
	 * Returns the next command with the following priority: 1. nextCommand object if set. 2. Front
	...
	/**
	 * Streams anything in the command buffer to the comm port. Synchronized to prevent commands
	 * from sending out of order.
	 */
	@Override
	synchronized public void streamCommands() {
		...
		while (this.getNextCommand() != null && CommUtils.checkRoomInBuffer(this.sentBufferSize, this.getNextCommand().getCommandString(), this.getBufferSize())
				&& allowMoreCommands()) {
	...
	/**
	 * Returns whether or not a completed command had an error based on a response from the
	 * controller.
	 * 
	 * @param response
	 * @return true if a command has completed.
